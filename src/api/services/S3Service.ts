...
export class S3Service {
    ...
    public createFolder(folderName: string = ''): Promise<any> {
        AWS.config.update({region: aws_setup.AWS_DEFAULT_REGION, accessKeyId: aws_setup.AWS_ACCESS_KEY_ID, secretAccessKey: aws_setup.AWS_SECRET_ACCESS_KEY});
        // Create the parameters for calling createBucket
        const bucketParams = {
            Bucket: aws_setup.AWS_BUCKET,
            Key: folderName,
        ...
        return new Promise((resolve, reject) => {
            return s3.getObject(bucketParams, (err: any, data: any) => {
                if (err) {
                    ...
                    resolve({ ...data, Body: data.Body.toString('binary'), status: 1 });
                }
            });
        });
    }
